...
ws_index = 0
# This part was added to only expose some properties/methods
class AbstractWebSocket:
	def __init__(self):
		global ws_index
		self.id: int = ws_index
		ws_index += 1
...
class WebSocket(AbstractWebSocket):
	...
	def send(self, data: str | bytes):
		...
		if self.wfile == None:
			raise ConnectionError("The websocket is already closed, you cannot send more messages!")
...
# One of these is instantiated every time a request comes in.
class HTTPRequestHandler(BaseHTTPRequestHandler):
	class NotWebsocketRequest(Exception):
		pass
	...
	def do_GET(self):
		...
		except HTTPRequestHandler.NotWebsocketRequest:
			return False
	...
	def onmessage(self, data: str):
		# print(repr(data))
		# self.send("its working wow!")
		pass
	...
	def opening_handshake(self):
		try:
			if self.request_version != "HTTP/1.1":
				raise HTTPRequestHandler.NotWebsocketRequest
